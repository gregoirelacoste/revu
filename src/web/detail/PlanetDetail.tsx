...
import type { FlatPlanet, Palette, EdgeData, FocusTarget, ReviewState } from '../types';
...
export function PlanetDetail({ planet, P, review, edges, allPlanets, onNavigate }: Props) {
  ...
  const activeItems = useMemo(() =>
    items.filter(m => review.flags[`${planet.id}-${m.name}`] == null),
    [items, planet.id, review.flags],
  );
  ...
  const maxU = Math.max(...items.map(m => m.usages || 1), 1);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize: 8, fontWeight: 800, color: iconC, fontFamily: MONO,
      }}>{ic.i}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: P.white, fontFamily: SANS }}>
      ...
      <span style={{ fontSize: 18, fontWeight: 900, color, fontFamily: MONO }}>
...
function Tag({ text, color }: { text: string; color: string }) {
  return (
    <span style={{
      fontSize: 7.5, fontFamily: MONO, color,
      ...
      {FLAGS.map(ft => {
        ...
        return (
          <button key={ft.key} onClick={() => toggleFlag(planet.id, ft.key)}
            style={{
              flex: 1, padding: '4px 0', borderRadius: 4, fontSize: 8, fontFamily: SANS,
    ...
    <div style={{ padding: '8px 14px', borderTop: `1px solid ${P.border}` }}>
      ...
      {linked.map(lp => (
        <div key={lp.id} onClick={() => onNavigate({ kind: 'planet', id: lp.id, planet: lp })}
          ...
          {...hoverBg(`${P.border}55`)}>
          <span style={{ fontWeight: 800, color: critPc(lp.crit, P), fontSize: 10, width: 28 }}>
            {lp.crit.toFixed(1)}
          </span>
          <span style={{ color: P.bright, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {lp.name}
          </span>
          <span style={{ color: P.dim, fontSize: 7 }}>{lp.galaxy.label}</span>
        </div>
      ))}
    </div>
  );
